...
def generate_section(headers_mapping: dict[str, str], rows, level=1, title=None, description=None, sort_by_lambda=None, sort_order='asc', filter_lambda=None):
    """
    ...
    for row in rows:
        section += "| " + " | ".join([str(row.get(val, "")) for val in headers_mapping.values()]) + " |\n"
